...
export function importWeekFromGGW(filePath) {
  ...
  for (const choferData of data.config.chofers) {
    ...
    try {
      ...
      const existing = db.getChofers().find(c => c.nombre.toUpperCase() === normalizedNombre);
      if (existing) {
  ...
  for (const vehiculoData of data.config.vehiculos) {
    ...
    try {
      const existing = db.getVehiculos().find(v => v.nombre.toUpperCase() === normalizedNombre);
      if (existing) {
